...
async def test_rename_own_session_ragflow_failure_still_updates_title(client, app, monkeypatch):
    """RAGFlow PATCH 失败时不阻塞,门户 title 仍更新(双写策略:失败不阻塞)。"""
    fake_session_id = "slice5-rename-001b"
    await _precreate_session(client, monkeypatch, fake_session_id)

    # mock RAGFlow PATCH 失败(抛异常)
    monkeypatch.setattr(
    ...
    assert resp.status_code == 200, f"RAGFlow 失败应不阻塞: {resp.text}"

    # 门户 title 仍更新
    owner = app.state.session_store.get(fake_session_id)
    assert owner is not None
    assert owner.title == "新标题"
...
async def test_hard_delete_user_ragflow_failure_marks_deleted(client, app, monkeypatch):
    """硬删除用户时 RAGFlow 删除失败 → session 标记 deleted_at(不阻塞用户删除)。"""
    await _login(client)
    alice = await _create_user_and_grant(client, username="alice")

    fake_session_id = "slice5-cascade-006b"
    _mock_precreate(monkeypatch, fake_session_id)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as alice_client:
    ...
    # 硬删除 alice(RAGFlow 失败不阻塞)
    resp = await client.delete(f"/admin/users/{alice['id']}")
    assert resp.status_code == 200, f"RAGFlow 失败应不阻塞用户删除: {resp.text}"

    # alice 已删除
    assert app.state.seed.get_user(alice["id"]) is None
    # session 标记 deleted_at(保留待重试)
    owner = app.state.session_store.get(fake_session_id)
    assert owner is not None
